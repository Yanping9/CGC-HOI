...
def crop(image, target, region):
    ...
    if "pose_bboxes" in target:
        boxes = target["pose_bboxes"]
        max_size = torch.as_tensor([w, h], dtype=torch.float32)
        cropped_boxes = boxes - torch.as_tensor([j, i, j, i])
        cropped_boxes = torch.min(cropped_boxes.reshape(-1, 2, 2), max_size)
        cropped_boxes = cropped_boxes.clamp(min=0)
        target["pose_bboxes"] = cropped_boxes.reshape(-1, 4)
...
def hflip(image, target):
    ...
    if "pose_bboxes" in target:
        ...
        if boxes.numel() == 0:
            ...
            with open("./output/no_bboxes.txt", "a") as f:
                log_message = f"Target: {target}\n"
                f.write(log_message)
            boxes = torch.tensor([0.0, 0.0, image.size[0], image.size[1]])
            if boxes.dim() == 1:
                boxes = boxes.unsqueeze(0)
        boxes = boxes[:, [2, 1, 0, 3]] * torch.as_tensor([-1, 1, -1, 1]) + torch.as_tensor([w, 0, w, 0])
        target["pose_bboxes"] = boxes
    if "boxes_o" in target:
        boxes = target["boxes_o"]
        boxes = boxes[:, [2, 1, 0, 3]] * torch.as_tensor([-1, 1, -1, 1]) + torch.as_tensor([w, 0, w, 0])
...
def resize(image, target, size, max_size=None):
    ...
    if "pose_bboxes" in target:
        ...
        if boxes.numel() == 0:
            # image_name = target["image_name"]
            with open("./no_bboxes.txt", "a") as f:
                log_message = f"Image name: {target}\n"
                f.write(log_message)
            boxes = torch.tensor([0.0, 0.0, image.size[0], image.size[1]])
            if boxes.dim() == 1:
                boxes = boxes.unsqueeze(0)
        scaled_boxes = boxes * torch.as_tensor([ratio_width, ratio_height, ratio_width, ratio_height])
        target["pose_bboxes"] = scaled_boxes
    if "boxes_o" in target:
        boxes = target["boxes_o"]
        scaled_boxes = boxes * torch.as_tensor([ratio_width, ratio_height, ratio_width, ratio_height])
