...
class HICODet(ImageDataset):
    ...
    def __getitem__(self, i: int) -> tuple:
        ...
        target = self._anno[intra_idx]
        if self.lsd_file is not None:
            lsd_image = self.load_image(os.path.join(
                self.lsd_file, self.filename(i)
            ))
        else:
            lsd_image = None
        if self.pose_file is not None:
            pose_features, pos_bboxes = self.get_pose_context(self.filename(i))
            target['pose_bboxes'] = pos_bboxes
        else:
            pose_features = None
        image = self.load_image(os.path.join(self._root, self._filenames[intra_idx]))
        return self._transforms(image, target, lsd_image, pose_features)
    ...
    def get_pose_context(self, image_name: str):
        with h5py.File(self.pose_file, 'r') as hf:
            group = hf[image_name]
            features = torch.from_numpy(group['features'][:]).float()
            bboxes = group['bboxes'][:].tolist()
