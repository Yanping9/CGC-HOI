...
from cgchoi import build_detector
...
def visualise_entire_image(image, output, attn, action=None, thresh=0.2):
    ...
    if action is not None:
        ...
        for i in keep:
            ho_pair_idx = output["x"][i] #attn (1,8,15,950), ho_pair_idx:tensor(2)
            attn_map = attn[0, :, ho_pair_idx].reshape(8, math.ceil(h / 32), math.ceil(w / 32))
            attn_image = image_copy.copy() #attn_map:(8,25,38)
...
def main(args):
    ...
    attn_weights = []  #list(1):(1,8,15,950)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resume', default='checkpoints/pvic-detr-r50-hicodet.pth', help='Resume from a model')
    parser.add_argument('--index', default=4050, type=int)
    parser.add_argument('--action', default=111, type=int,
