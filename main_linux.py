...
def main(rank, args):
    ...
    trainset = DataFactory(
        ...
        data_root=args.data_root, enhance_mode=args.use_lsdpose
    )
    testset = DataFactory(
    ...
    if args.eval:
        if args.dataset == 'vcoco':
            ...
            if rank == 0:
                ...
                results = {
                    "mAP": {
                    "full": round(ap.mean(), 4),
                    "rare": round(ap[rare].mean(), 4),
                    "none-rare": round(ap[non_rare].mean(), 4)
                    }
                }
                with open('output/hico_results.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_lsdpose', default=True, type=bool)
