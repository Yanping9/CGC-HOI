...
# def match_pose2region(h_bboxes, pose):
#     pose_features = pose["features"]
#     pose_bboxes = pose["bboxes"]
#     poseIdex = torch.zeros(h_bboxes.shape[0]).to(torch.long)
#     for i, h_box in enumerate(h_bboxes):
#         poseIdex[i] = find_most_similar_bbox(h_box, pose_bboxes)
#     # print(poseIdex)
#     return pose_features[poseIdex], poseIdex
def match_pose2region(boxes, lables, pose):
    mask = lables == 0
    h_bboxes = boxes[mask]
    pose_features = pose["features"]
    pose_bboxes = pose["bboxes"]
...
def find_most_similar_bbox(a, b):
    """
    找到b中与a最相似的边界框的索引

    参数:
        a: torch.Tensor, 形状为(1,4)的边界框，格式为[x1, y1, x2, y2]
        b: torch.Tensor, 形状为(n,4)的边界框集合，每行格式为[x1, y1, x2, y2]

    返回:
        int: b中与a IoU最大的边界框的索引
    """
    # 确保输入形状正确
    # assert a.shape == (1, 4), "a的形状应为(1,4)"
    assert len(b.shape) == 2 and b.shape[1] == 4, "b的形状应为(n,4)"
    ...
    # 找到最大IoU的索引
    max_idx = torch.argmax(ious).item()
...
def bbox_iou(box1, box2):
    ...
    参数:
        box1: torch.Tensor, 形状为(1,4)或(n,4)
        box2: torch.Tensor, 形状为(m,4)

    返回:
        torch.Tensor: IoU值，形状取决于输入广播后的形状
    """
    # 获取交集的坐标
