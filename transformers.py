...
class HORefineLayer(nn.Module):
    ...
    def __init__(self, q_dim, kv_dim, num_heads, ffn_interm_dim, dropout=0.1):
        ...
        # Linear projections on qkv have been removed in this custom layer.
        self.q_attn_hum = MultiheadAttention(q_dim, num_heads, dropout=dropout)
        self.q_attn_obj = MultiheadAttention(q_dim, num_heads, dropout=dropout)
        # Add the missing linear projections.
        self.q_attn_q_proj_hum = nn.Linear(q_dim, q_dim)
        self.q_attn_k_proj_hum = nn.Linear(q_dim, q_dim)
        self.q_attn_v_proj_hum = nn.Linear(q_dim, q_dim)
        self.q_attn_q_proj_obj = nn.Linear(q_dim, q_dim)
        self.q_attn_k_proj_obj = nn.Linear(q_dim, q_dim)
        self.q_attn_v_proj_obj = nn.Linear(q_dim, q_dim)
        # Each scalar is mapped to a vector of shape kv_dim // 2.
        # For a box pair, the dimension is 8 * (kv_dim // 2).
        self.q_attn_qpos_proj_hum = nn.Linear(q_dim, q_dim)
        self.q_attn_kpos_proj_hum = nn.Linear(q_dim, q_dim)
        self.q_attn_qpos_proj_obj = nn.Linear(q_dim, q_dim)
        self.q_attn_kpos_proj_obj = nn.Linear(q_dim, q_dim)
        ...
        self.ln2 = nn.LayerNorm(q_dim)
        self.ln3 = nn.LayerNorm(q_dim)
        self.ln4 = nn.LayerNorm(q_dim)
        self.ln5 = nn.LayerNorm(q_dim)
        self.dp1 = nn.Dropout(dropout)
        self.dp2 = nn.Dropout(dropout)
        self.dp3 = nn.Dropout(dropout)
    ...
    def forward(self,
            queries_hum: Tensor, queries_obj: Tensor, pose_feas: Tensor,
            pos_hum: Tensor, pos_obj: Tensor,
            q_attn_mask: Optional[Tensor] = None,
            qk_attn_mask: Optional[Tensor] = None,
            q_padding_mask: Optional[Tensor] = None,
            kv_padding_mask: Optional[Tensor] = None,
        ...
        # Perform self attention amongst queries
        q_hum = self.q_attn_q_proj_hum(queries_hum)
        k_hum = self.q_attn_k_proj_hum(queries_hum)
        v_hum = self.q_attn_v_proj_hum(queries_hum)
        q_p_hum = self.q_attn_qpos_proj_hum(pos_hum)
        k_p_hum = self.q_attn_kpos_proj_hum(pos_hum)
        q_hum = q_hum + q_p_hum
        k_hum = k_hum + k_p_hum
        q_attn_hum = self.q_attn_hum(
            q_hum, k_hum, value=v_hum, attn_mask=q_attn_mask,
            key_padding_mask=q_padding_mask
        )[0]
        queries_hum = self.ln1(queries_hum + self.dp1(q_attn_hum))
        # Perform cross attention from memory features to queries
        q_hum = self.qk_attn_q_proj(queries_hum)
        k = self.qk_attn_k_proj(pose_feas)
        v = self.qk_attn_v_proj(pose_feas)
        ...
        qk_attn = self.qk_attn(
            query=q_hum, key=k, value=v, attn_mask=qk_attn_mask,
            key_padding_mask=kv_padding_mask
        )[0]
        queries_hum = self.ln2(queries_hum + self.dp2(qk_attn))
        queries_hum = self.ln3(queries_hum + self.dp3(self.ffn1(queries_hum)))

        q_obj = self.q_attn_q_proj_obj(queries_obj)
        k_obj = self.q_attn_k_proj_obj(queries_obj)
        v_obj = self.q_attn_v_proj_obj(queries_obj)
        q_p_obj = self.q_attn_qpos_proj_obj(pos_obj)
        k_p_obj = self.q_attn_kpos_proj_obj(pos_obj)
        q_obj = q_obj + q_p_obj
        k_obj = k_obj + k_p_obj
        q_attn_obj = self.q_attn_obj(
            q_obj, k_obj, value=v_obj, attn_mask=q_attn_mask,
            key_padding_mask=q_padding_mask
        )[0]
        queries_obj = self.ln4(queries_obj + self.dp4(q_attn_obj))
        queries_obj = self.ln5(queries_obj + self.dp5(self.ffn2(queries_obj)))
        return queries_hum, queries_obj

class HORefineDecoder(nn.Module):

    def __init__(self, num_layers, return_intermediate=False, hidden_size=256, num_heads=8, dropout=0.1):
        super().__init__()
        self.layers = nn.ModuleList([HORefineLayer(
            q_dim=hidden_size, kv_dim=hidden_size,
            num_heads=num_heads, ffn_interm_dim=hidden_size * 4, dropout=dropout
        ) for _ in range(num_layers)])
        self.num_layers = num_layers
    ...
    def forward(self, queries_hum, queries_obj, pose_feas, pos_hum, pose_obj,
            q_attn_mask: Optional[Tensor] = None,
            qk_attn_mask: Optional[Tensor] = None,
            q_padding_mask: Optional[Tensor] = None,
            kv_padding_mask: Optional[Tensor] = None,
        ...
        # Add support for zero layers
        if self.num_layers == 0:
            return queries_hum.unsqueeze(0), queries_obj.unsqueeze(0)
        # Explicitly handle zero-size queries
        if queries_hum.numel() == 0 or queries_obj.numel() == 0:
            rp = self.num_layers if self.return_intermediate else 1
            return queries_hum.unsqueeze(0).repeat(rp, 1, 1, 1), queries_obj.unsqueeze(0).repeat(rp, 1, 1, 1)

        output_hum = queries_hum
        output_obj = queries_obj
        intermediate_hum = []
        intermediate_obj = []
        for layer in self.layers:
            output_hum, output_obj = layer(
                output_hum, output_obj, pose_feas, pos_hum, pose_obj,
                q_attn_mask=q_attn_mask,
                qk_attn_mask=qk_attn_mask,
                q_padding_mask=q_padding_mask,
                kv_padding_mask=kv_padding_mask,
            ...
            if self.return_intermediate:
                intermediate_hum.append(self.norm(output_hum))
                intermediate_obj.append(self.norm(output_obj))

        if self.return_intermediate:
            output_hum = torch.stack(intermediate_hum)
            output_obj = torch.stack(intermediate_obj)
        else:
            output_hum = self.norm(output_hum).squeeze(1)
            output_obj = self.norm(output_obj).squeeze(1)
