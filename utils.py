...
def log_json_to_file(message, filename=""):
    with open(filename, 'a', encoding='utf-8') as f:
        f.write(json.dumps(message) + "\n")

def custom_collate(batch, enhance_mode=True):
    images = []
    targets = []
    if enhance_mode:
        lsd_images = []
        pose_features = []
        for im, lsd_im, pose, tar in batch:
            images.append(im)
            lsd_images.append(lsd_im)
            pose_features.append(pose)
            targets.append(tar)
    else:
        for im, _, _, tar in batch:
            images.append(im)
            targets.append(tar)
        lsd_images = None
        pose_features = None
    return images, lsd_images, pose_features, targets
...
class DataFactory(Dataset):
    def __init__(self, name, partition, data_root, enhance_mode: bool):
        ...
        if name == 'hicodet':
            ...
            pose_dir = dict(
                train2015="hico_20160224_det/images/pose/train2015.h5",
                test2015="hico_20160224_det/images/pose/test2015.h5"
            )
            lsd_image_dir = dict(
                train2015='hico_20160224_det/images/lsd_train2015',
                test2015='hico_20160224_det/images/lsd_test2015'
            )
            ...
            self.dataset = HICODet(
                ...
                pose_file=os.path.join(data_root, pose_dir[partition]),
                lsd_file=os.path.join(data_root, lsd_image_dir[partition])
            )
        else:
            ...
            pose_dir = dict(
                train='mscoco2014/pose/train2014.h5',
                val='mscoco2014/pose/train2014.h5',
                trainval='mscoco2014/pose/train2014.h5',
                test='mscoco2014/pose/val2014.h5'
            )
            lsd_image_dir = dict(
                train='mscoco2014/lsd_train2014',
                val='mscoco2014/lsd_train2014',
                trainval='mscoco2014/lsd_train2014',
                test='mscoco2014/lsd_val2014'
            )
            if enhance_mode:
                self.dataset = VCOCO(
        ...
        normalize_lsd = T.Compose([T.ToTensor()])
        ...
        self.lsd_transforms = T.Compose([normalize])
        self.name = name
    ...
    def __getitem__(self, i):
        image, lsd_image, pose_fea, target  = self.dataset[i]
        if pose_fea is not None:
            pose = dict()
            pose['features'] = pose_fea
            pose['bboxes'] = target['pose_bboxes']
            # target.pop('pose_bboxes')
        else:
            pose = None
        ...
        image, target = self.transforms(image, target)#处理输入的原图->tensor
        # pose["bboxes"] = target["pose_bboxes"]
        pose["bboxes"] = recover_boxes(target["pose_bboxes"], target["size"])
        target.pop("pose_bboxes")
        if lsd_image is not None:
            lsd_image = self.lsd_transforms(lsd_image, None)[0]

        return image, lsd_image, pose, target
...
class CustomisedDLE(DistributedLearningEngine):
    ...
    def _print_statistics(self):
        ...
        if self._rank == 0:
            ...
            log_entry = {
                ...
                "iter_in_epoch": str(self._state.iteration - num_iter * (self._state.epoch - 1)).zfill(n_d),
                "total_iter_per_epoch": num_iter,
                "loss": float(running_loss),  # 明确转为 float，避免后续 JSON 序列化问题
                "datasets": self._train_loader.dataset.name
            }
            log_json_to_file(log_entry, "./output/logs/training_log.json")
            wandb.log({
    ...
    def _on_end_epoch(self):
        if self._train_loader.dataset.name == "hicodet":
            ...
            if self._rank == 0:
                ...
                log_json_to_file(log_entry, "./output/logs/training_hico_map_log.json")
                wandb.log({
                    "epochs": self._state.epoch, "mAP full": perf[0],
                    "mAP rare": perf[1], "mAP non_rare": perf[2]
                })
            ...
            if self._rank == 0:
                ...
                log_json_to_file(log_entry, "./output/logs/training_map_log.json")
                """
                NOTE wandb was not setup for V-COCO as the dataset was only used for evaluation
                """
    ...
    def cache_hico(self, dataloader, cache_dir='matlab'):
        ...
        for i, (image, line, pose, target) in enumerate(tqdm(dataloader.dataset)):
            inputs = pocket.ops.relocate_to_cuda([image,])
            line = pocket.ops.relocate_to_cuda([line,])
            pose = pocket.ops.relocate_to_cuda([pose,])
            output = net(inputs, line, pose)
