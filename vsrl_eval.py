...
class VCOCOeval(object):
    ...
    def _do_eval(self, detections_file, output_json_path, ovr_thresh=0.5):
        results = []
        vcocodb = self._get_vcocodb()
        # self._do_agent_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh)
        result_role_ap_1 = self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type='scenario_1')
        results.append(result_role_ap_1)
        result_role_ap_2 = self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type='scenario_2')
        results.append(result_role_ap_2)
    ...
    def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
        ...
        for aid in range(self.num_actions):
            ...
            for rid in range(len(self.roles[aid])-1):
                ...
                role_ap[aid, rid] = voc_ap(rec, prec, self.eps)
        test_result = {
            'Method': 'Role_AP_' + eval_type,
            'Each_AP': []
        }
        ...
        for aid in range(self.num_actions):
            ...
            for rid in range(len(self.roles[aid])-1):
                name = self.actions[aid] + '-' + self.roles[aid][rid+1]
                ap = role_ap[aid, rid]*100.0
                pos = int(npos[aid])
                print('{: >23}: AP = {:0.2f} (#pos = {:d})'.format(name, ap, pos))
                ap = "{:0.2f}".format(ap)
                pos = "{:d}".format(pos)
                test_result['Each_AP'].append({name: ap, 'pos': pos})
        average_ap = np.nanmean(role_ap)*100.00
        print('Average Role [%s] AP = %.2f'%(eval_type, average_ap))
        test_result['Average_AP'] = average_ap
        print('---------------------------------------------')
        role_ap[-3][0] = np.nan # remove point_inst
        average_ap = np.nanmean(role_ap)*100.00
        print('Average Role [%s] AP = %.2f, omitting the action "point"'%(eval_type, average_ap))
        test_result['Average_AP_omit_point'] = average_ap
        print('---------------------------------------------')
        return test_result
...
def voc_ap(rec, prec, eps):
    ...
    ap = np.sum((mrec[i + 1] - mrec[i]) * mpre[i + 1])*(1+eps)
    return ap
...
if __name__ == '__main__':
    ...
    vsrl_annot_file = 'v-coco/data/vcoco/vcoco_test.json'
    coco_file = 'v-coco/data/instances_vcoco_all_2014.json'
    split_file = 'v-coco/data/splits/vcoco_test.ids'
    output_json_path = 'output/union_pos_b4.json'
    vcocoeval = VCOCOeval(vsrl_annot_file, coco_file, split_file)

    det_file = 'output/union_pos_b4.pickle'
    vcocoeval._do_eval(det_file, output_json_path, ovr_thresh=0.5)